...
import { getCredentials as getSQLiteCredentials, insertMTR, updateMTRStatus } from "../databases/database";
...
/**
 * Abre o PDF da MTR em um visualizador externo
 */
export const openMTRPDF = async (mtrId: string): Promise<void> => {
  try {
    // Usando a URL do servidor remoto conforme solicitado
    const baseURL = "http://159.89.191.25:8000";

    // Construir a URL diretamente para o endpoint de download
    const downloadUrl = `${baseURL}/docs/swagger/mtr/${mtrId}/download`;
    ...
    if (supported) {
      ...
      throw new Error(`Não é possível abrir a URL: ${downloadUrl}`);
    }
  } catch (error: any) {
